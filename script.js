...
// });
submitDeathYear.addEventListener('click', () => {
  const deathDateInput = deathYearInput.value.trim(); // Get the input value

  // Validate the input format "dd-mm-yyyy"
  if (isValidDate(deathDateInput)) {
    const parsedDate = parseDate(deathDateInput);
    ...
    if (year >= 2024 && year <= 2100) {
      deathYear = year; // Update the deathYear
      deathMonth = month; // Update the deathMonth (if needed)
      deathDay = day; // Update the deathDay (if needed)
      
      localStorage.setItem('deathYear', year.toString());
      localStorage.setItem('deathMonth', month.toString());
      localStorage.setItem('deathDay', day.toString());
...
// Function to validate date input in "dd-mm-yyyy" format
function isValidDate(dateString) {
  const regex = /^\d{2}-\d{2}-\d{4}$/; // Check format "dd-mm-yyyy"
...
function updateCountdownTimer() {
  if (!deathYear || !deathMonth || !deathDay) return; // Make sure deathYear, deathMonth, and deathDay are set
  
  const now = new Date();

  // Create a new Date object using deathYear, deathMonth, and deathDay
  // Note that months are 0-indexed in JavaScript, so subtract 1 from deathMonth
  const deathDate = new Date(deathYear, deathMonth - 1, deathDay); 

  const timeLeft = deathDate - now; // Calculate the difference between now and the death date
